...
2. **parking_positions** - A collection that stores the list of potential parking positions that are later selected as Free Parking Positions. Each element in the list is generated by identifying a car entering into the parking area and choosing it's optimal position that is within the boundries of the parking area.
...
2. **Parking Model** - identifies Occopied Parking Positions using Yolov5 model and selecting the Max Number of possible Available Parking positions from "parking_positions" that is generated by Parking Position Detector Class.  
