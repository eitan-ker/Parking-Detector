...
**parking_areas** is the only data that is manually generated by the user. (The rest is Automated).\ **parking_positions** is a collection that stores the list of potential parking positions that are later selected as Free Parking Positions. Each element in the list is generated by identifying a car entering the parking area and choosing it's optimal position that is within the boundries of the parking area.
